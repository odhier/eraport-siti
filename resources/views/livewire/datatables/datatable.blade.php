<div>
    ...
    <div class="relative">
        <div class="flex justify-between items-center mb-1">
            <div class="flex-grow h-10 flex items-center space-x-1">
                ...
                <div class="w-96 flex rounded-lg shadow-sm">
                    <div class="relative flex-grow focus-within:z-10">
                        ...
                        <input wire:model.debounce.500ms="search" class="form-input py-1 pr-5 block bg-gray-50 focus:bg-white outline-teal-500 w-full rounded-md pl-10 transition ease-in-out duration-150 sm:text-sm sm:leading-5" placeholder="Search in {{ $this->searchableColumns()->map->label->join(', ') }}" />
                ...
                @endif
                @isset($select_semester)
                @if($select_semester)
                <div class="w-96 flex rounded-lg shadow-sm">
                    <div class="relative flex-grow focus-within:z-10 focus:outline-none">
                        <select wire:model="selected_semester" class="form-input py-0 px-1 block bg-gray-50 focus:outline-none focus:shadow-outline focus:border-teal-300 text-white bg-teal-500 border-opacity-50 border-teal-300 w-full rounded-md pl-10 transition ease-in-out duration-150 sm:text-sm sm:leading-5" style="width:100px;height: calc(1.6em + 6px);">
        ...
        <div class="p-2 grid grid-cols-8 gap-2">
            ...
            {{ $column['hidden'] ? 'bg-teal-100 hover:bg-teal-300 text-blue-600' : 'bg-teal-500 hover:bg-teal-600' }}">
                {{ $column['label'] }}
            </button>
            @endforeach
        </div>
        ...
        <div class="rounded-lg bg-white max-w-screen overflow-x-scroll">
                ...
                <div class="table align-middle min-w-full">
                    ...
                    <div class="table-row divide-x divide-gray-200">
                        @foreach($this->columns as $index => $column)
                            ...
                            <div class="relative table-cell h-12 w-32 overflow-hidden align-top px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider flex items-center focus:outline-none">
                    ...
                    <div class="table-row divide-x divide-blue-200 bg-blue-100">
                        @foreach($this->columns as $index => $column)
                            @if($column['hidden'])
                                ...
                                @endif
                            @elseif($column['type'] === 'checkbox')
                                <div class="w-32 overflow-hidden align-top bg-blue-100 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider flex h-full flex-col items-center space-y-2 focus:outline-none">
                                    <div>SELECT ALL</div>
                                    <div>
                                        <input type="checkbox" wire:click="toggleSelectAll" @if(count($selected) === $this->results->total()) checked @endif class="form-checkboxh-4 w-4 text-blue-600 transition duration-150 ease-in-out" />
                                    </div>
                                </div>
                            @else
